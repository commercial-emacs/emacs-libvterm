...
{
  const int o_nrows = screen->rows, o_ncols = screen->cols;

  ScreenCell *o_buffer = screen->buffers[bufidx];
  VTermLineInfo *o_lineinfo = statefields->lineinfos[bufidx];
  ...
  while(o_irow >= 0) {
	...
	(cursor.row - logic_first) < nrows))
    {
      ...
      int rowcount = nrows - downwards;

#ifdef DEBUG_REFLOW
      fprintf(stderr, "  scroll %d rows +%d downwards\n", rowcount, downwards);
#endif

      memmove(&buffer[downwards * ncols], &buffer[0], rowcount * ncols * sizeof(ScreenCell));
      memmove(&lineinfo[downwards], &lineinfo[0], rowcount * sizeof(lineinfo[0]));

      irow += downwards;
      logic_first += downwards;
      logic_last += downwards;
	...
	++irow) {
      ...
      for (; icol < MIN(o_nchars, ncols); ++icol) {
        ...
        if(o_cursor.row == o_irow && o_cursor.col == o_icol)
          cursor.row = irow, cursor.col = icol;
      ...
      while(icol < ncols) {
        clearcell(screen, &buffer[irow * ncols + icol]);
        icol++;
      }

      lineinfo[irow].continuation = (irow > logic_first);
    }

    o_irow = o_logic_first - 1;
    if (cursor.row < 0 && o_cursor.row > o_irow) {
      int linear = (o_cursor.row - o_logic_first) * o_ncols + o_cursor.col;
      cursor.row = logic_first + linear / ncols;
      cursor.col = linear % ncols;
    }
    irow = logic_first - 1;
  }

  if(o_cursor.row <= o_irow) {
    /* bring back a cursor scrolled off the top */
    cursor.row = 0, cursor.col = o_cursor.col;
  }
  ...
  if(irow >= 0 &&
    ...
    while(irow >= 0) {
      ...
      VTermPos pos = { .row = irow };
      for(pos.col = 0; pos.col < o_ncols && pos.col < ncols; pos.col += screen->sb_buffer[pos.col].width) {
        VTermScreenCell *src = &screen->sb_buffer[pos.col];
        ScreenCell *dst = &buffer[pos.row * ncols + pos.col];
        ...
        if(src->width == 2 && pos.col < (ncols-1))
          (dst + 1)->chars[0] = (uint32_t) -1;
      }
      for( ; pos.col < ncols; pos.col++)
        clearcell(screen, &buffer[pos.row * ncols + pos.col]);
      irow--;
  ...
  if(irow >= 0) {
    /* Still empty lines up top.  Shift up contents. */
    int moverows = nrows - irow - 1;
    memmove(&buffer[0], &buffer[(irow + 1) * ncols], moverows * ncols * sizeof(ScreenCell));
    memmove(&lineinfo[0], &lineinfo[irow + 1], moverows * sizeof(lineinfo[0]));

    cursor.row -= (irow + 1);

    for(irow = moverows; irow < nrows; irow++) {
      for(int col = 0; col < ncols; col++)
        clearcell(screen, &buffer[irow * ncols + col]);
      lineinfo[irow] = (VTermLineInfo){ 0 };
    }
  }

  vterm_allocator_free(screen->vt, o_buffer);
  screen->buffers[bufidx] = buffer;

  vterm_allocator_free(screen->vt, o_lineinfo);
  statefields->lineinfos[bufidx] = lineinfo;
